...
- **Date arithmetic** — no date library is pinned in dean-stack. Surface a stack-pin decision with the user before adding `date-fns` or anything else; `Intl.DateTimeFormat` covers display-only needs.
...
- **Don't import `bcrypt` / `argon2`** — there is no server in dean-stack; auth doesn't apply on GH Pages.
