...
- The `.github/workflows/sonarjs.yml` workflow shape (PR-only, `head_ref` pinned, `bun --filter=@dean-stack/web run check:sonar`).
...
## Calibrated rule policy (for dean-stack's domain)
...
- **`sonar-scanner`, `sonarqube-scanner`, `@sonar/scan`, `host.url`, `sonar.token`.** All require a SonarQube/SonarCloud server. dean-stack is server-less.
...
6. Run `bun --filter=@dean-stack/<name> run check:sonar` once and triage findings: real bugs first, then refactor patterns from the table above, then policy adjustments (with rationale comments) for irreducible cases.
