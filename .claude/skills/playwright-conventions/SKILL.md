...
Sub-skill of `playwright`. The **canonical home of the ASK-FIRST rule** plus every cross-cutting Playwright convention in dean-stack: selector preference order, fixture patterns (fresh IDB / seeded IDB / offline / throttled), reduced-motion forcing, the `playwright.config.ts` shape, and the unit-vs-E2E partition. The four other Playwright sub-skills defer here for these contracts.
...
- Pillar 4 (CLI-gate-first) means: Playwright is the final stage of `bun run check` — `biome ci → stylelint → tsgo --noEmit → bun test → playwright test`. A failing Playwright run is a red gate. Never disable, never `test.skip` to make CI green.
...
// XPath — only for legacy DOM you can't change. Almost never needed in dean-stack.
```
...
expect(await page.title()).toBe("dean-stack");
