# Ownership Matrix — dean-stack skill system
...
| nitro | narrow | — | Nitro v3 `static` / `github_pages` preset, `nitro.config.ts`, prerender configuration, base-path handling, and the SPA-fallback `404.html` that GH Pages serves. | nitro, nitro.config, nitro preset, github_pages preset, prerender, static preset, baseURL, SPA fallback 404 |
...
| `@theme` | tailwind |
| `.nvmrc` | node |
| `addChild` | (out of scope — pixijs collection, not dean-stack) |
| `anime v4` | animejs |
| `anime utils` | animejs |
| `anime.js` | animejs |
| `animejs` | animejs |
...
(Note: the row for `addChild` is intentionally listed as out-of-scope to flag a potential collision with the existing `pixijs-scene-container` skill — that keyword stays with pixijs and is not claimed by any dean-stack skill.)
...
- **dean-stack rule:** runtime parsing is dev-only via `defineComponent(schema, fn)`; production tree-shakes the parse call.
...
- **dean-stack rule:** the root `idbHydrationPromise` resolves once; `atomWithIDB` reads sync after that.
...
### jotai (latest 2.x)
- `atom`, `useAtom`, `useAtomValue`, `useSetAtom`, `useStore`, `Provider`, `atomWithStorage`, `loadable`, `unwrap`, `useHydrateAtoms`. (`atomFamily` is intentionally excluded — dean-stack uses the `atomWithIDB` key + module-scope `Map<id, atom>` pattern; see `jotai/SKILL.md` parameterized-atoms section.)
...
### nitro (Nitro v3)
- `defineNitroConfig({ preset: "github_pages", baseURL: "/dean-stack/", prerender: { routes: [...] } })`.
...
- **dean-stack rule:** every type that has a Zod schema is `z.infer<typeof Schema>` — never hand-written.
...
- **dean-stack rule:** ASK FIRST before writing any Playwright test. The user owns the structural decisions.
