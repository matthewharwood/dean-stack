...
test("save writes a row to the progress object store", async ({ page }) => {
  ...
  const rows = await page.evaluate(async () => {
    const open = indexedDB.open("dean-stack");
...
const test = base.extend<{ seededPage: typeof base extends never ? never : Awaited<ReturnType<typeof base["page"]["fixture"]>> }>({
  ...
  seededPage: async ({ page }, runFixture) => {
    ...
    await page.addInitScript(() => {
      const open = indexedDB.open("dean-stack", 2);
...
```
`context.newPage()` shares a `BrowserContext` (and therefore the `BroadcastChannel`) across pages. See `idb` for the channel name (`dean-stack:idb`) and the message shape.
