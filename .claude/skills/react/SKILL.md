...
- `idb` — for the dean-stack root hydration contract (`idbHydrationPromise`, root Suspense, debounced write-through, `BroadcastChannel`).
...
| "How do I read the IDB hydration promise inside the shell?" | `react-19-primitives` (API) + `idb` (dean-stack wiring) |
