...
- **Don't switch to `injectManifest` strategy** unless you have a specific reason — `generateSW` produces the right Workbox config for the dean-stack contract; `injectManifest` requires writing the SW by hand and re-implementing the navigation route.
