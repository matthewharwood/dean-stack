...
- Node `>=20.19 || >=22.12` for Storybook 10 (the dean-stack pin is Node 25 — fine).
...
stories: [
  ...
  // No MDX docs in the dean-stack story glob — keep it minimal. Add `*.mdx` only when a docs-only page exists.
];
```
