...
### Workspace script
Stylelint 16 dropped the native `--watch` flag. dean-stack wraps Stylelint in `chokidar-cli` (root `devDependencies`) so the CLI watcher is real and the IDE plugin stays non-authoritative.
