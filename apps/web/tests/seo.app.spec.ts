// ASK-FIRST decisions:
//   1. Level: app (real prerendered HTML + SPA hydration on :3000).
//   2. Assertion: title, description, og:*, twitter:*, canonical, JSON-LD on
//      `/` (defaults) and `/games/maze/3` (per-route override). Plus /robots.txt,
//      /llms.txt, /sitemap.xml return 200 with non-empty bodies.
...
test("home (/) renders default SEO meta + JSON-LD", async ({ page }) => {
  ...
  await expect(page).toHaveTitle(/dean-stack/i);

  const description = await metaContent(page, 'meta[name="description"]');
  expect(description.length).toBeGreaterThanOrEqual(40);

  expect(await metaContent(page, 'meta[property="og:title"]')).toMatch(/dean-stack/i);
  ...
  expect(await metaContent(page, 'meta[name="twitter:title"]')).toMatch(/dean-stack/i);
...
test("/games/maze/3 overrides title + canonical via per-route head", async ({ page }) => {
  await page.goto("/games/maze/3");

  await expect(page).toHaveTitle(/Maze.*Level 3/i);

  const description = await metaContent(page, 'meta[name="description"]');
  expect(description).toMatch(/level 3/i);

  expect(await metaContent(page, 'meta[property="og:title"]')).toMatch(/Maze.*Level 3/i);

  const canonical = await page.locator('head link[rel="canonical"]').first().getAttribute("href");
  expect(canonical).toMatch(/\/games\/maze\/3$/);
});
...
test("/llms.txt is served with project description", async ({ page }) => {
  ...
  expect(body).toMatch(/dean-stack/i);
});
